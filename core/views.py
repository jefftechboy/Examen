...
class HistorialComprasViewSet(viewsets.ModelViewSet):
    queryset = HistorialCompras.objects.all().order_by('id')
...
def cuentaCliente(request):
    historial = HistorialCompras.objects.all()  # Obtener todos los objetos de HistorialCompras
    aux = {'compras': historial}  # Crear el diccionario con los objetos para pasar a la plantilla
...
def carroCompra(request):
    ...
    aux = {
        'historial': HistorialComprasForm()
    }
    ...
    if request.method == 'POST':
        ...
        if formulario.is_valid():
            ...
            return render(request, 'core/cliente.html', aux)  # Redirige a donde corresponda después de guardar

    else:
        formulario = HistorialComprasForm(initial={'Usuario': request.user.username, 'fechaCompra': fecha_actual})
...
def login_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                return redirect('nombre_de_tu_vista_principal')  # Cambia 'nombre_de_tu_vista_principal' por la vista a la que quieres redirigir tras el login
    else:
        form = LoginForm()
    ...
    return render(request, 'tu_template_de_login.html', context)
@login_required
...
def agregarAutentificacion(request):
    ...
    if request.method == 'POST':
        ...
        formulario2 = HistorialComprasForm(data=request.POST, files=request.FILES)
        if formulario.is_valid():
            formulario.save()
            ...
            messages.success(request, "Autentificación agregada correctamente")
            return render(request, 'core/inicio.html', aux)
        else:
            aux['form'] = formulario
...
def eliminar_compras(request):
    if request.method == 'POST':
        ...
        Compra.objects.all().filter(Usuario=username).delete()
        return render(request,'core/carroCompra.html')
    return render(request,'core/carroCompra.html')

# REGISTRAR CUENTA
def registro(request,user):
    data = {
...
def registro(request):
    ...
    if request.method == 'POST':
        formulario = CustomUserCreationForm(data=request.POST)
        if formulario.is_valid():
            usuario_nuevo = formulario.save(commit=False)
            ...
            # Autenticación del usuario después de registrarse
            user = None
            
            # Intenta autenticar primero por username
            user = authenticate(request=request, username=username, password=password)
            
            # Si no se autentica por username, intenta por email
            ...
            if user is not None:
                ...
                group_clientes = Group.objects.get(name='clientes')
                usuario_ingreso.groups.add(group_clientes)
